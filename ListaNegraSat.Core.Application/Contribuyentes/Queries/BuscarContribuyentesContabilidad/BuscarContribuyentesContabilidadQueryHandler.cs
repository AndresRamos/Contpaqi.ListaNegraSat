...
{
    public class BuscarContribuyentesContabilidadQueryHandler : IRequestHandler<BuscarContribuyentesContabilidadQuery, BuscarContribuyentesContabilidadResult>
    {
        private readonly IMediator _mediator;
        private readonly IContribuyenteContabilidadRepository _contribuyenteContabilidadRepository;

        public BuscarContribuyentesContabilidadQueryHandler(IMediator mediator, IContribuyenteContabilidadRepository contribuyenteContabilidadRepository)
        {
            _mediator = mediator;
            _contribuyenteContabilidadRepository = contribuyenteContabilidadRepository;
        }

        public async Task<BuscarContribuyentesContabilidadResult> Handle(BuscarContribuyentesContabilidadQuery request, CancellationToken cancellationToken)
        {
            var archivo = await _mediator.Send(new BuscarContribuyentes69BQuery(request.FileName), cancellationToken);

            var contribuyentsContabilidad = await _contribuyenteContabilidadRepository.BuscarTodosAsync(cancellationToken);

            var contribuyentesContabilidad69B = new List<ContribuyenteContabilidadDto>();

            foreach (var personaContabilidad in contribuyentsContabilidad)
            {
                var contribuyente69B = archivo.Contribuyentes.FirstOrDefault(c => c.Rfc == personaContabilidad.Rfc);
                ...
                personaContabilidad.ListadoNumeroYFechaDeOficioGlobalDeContribuyentesQueDesvirtuaron = contribuyente69B.NumeroYFechaDeOficioGlobalDeContribuyentesQueDesvirtuaron;
                ...
                personaContabilidad.ListadoNumeroYFechaDeOficioGlobalDeSentenciaFavorable = contribuyente69B.NumeroYFechaDeOficioGlobalDeSentenciaFavorable;
            ...
            var result = new BuscarContribuyentesContabilidadResult {Version = archivo.Version, Contribuyentes = contribuyentesContabilidad69B};
